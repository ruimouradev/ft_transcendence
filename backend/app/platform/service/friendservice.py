...
def get_suggested_friends(session: Session, current_user: CurrentUser)-> list[Friend]:
    ...
    '''
    statement = select(User,UserStatistic).join(UserStatistic, User.id == UserStatistic.user_id).where(User.id != current_user.id,User.is_superuser == False,User.is_active == True,
                                    User.id.not_in(
    ...
    user_requested_result = session.exec(
        select(User, UserStatistic).join(Friendship, and_(User.id == Friendship.addressee_id, Friendship.status == FriendshipStatus.PENDING,Friendship.requester_id == current_user.id)).join(UserStatistic, User.id == UserStatistic.user_id).where(User.is_superuser == False,User.is_active == True)
    ).all()
