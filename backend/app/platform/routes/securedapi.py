...
@router.get("/onlineplayers", response_model=list[UserOnLineStatus], dependencies=[Depends(verify_api_key)],summary="An secured API endpoint,get online players", description='''This endpoint requires a valid API key and client ID to access.
...
@router.get("/{user_id}/friendlistwithrank", response_model=list[UserStatisticLeaderboardEntry], dependencies=[Depends(verify_api_key)], summary="An secured API endpoint, get specific user friend list with rank", description='''Retrieve the friend list of a specific user along with their ranks. Requires a valid API key and client ID.
To obtain an API key and client ID, use the `/users/apikey` endpoint. Include the following headers in your request:
- `X-API-Key`: Your API key
- `X-Client-ID`: Your client ID
Example:
...
curl -X 'GET' \n
  ...
  -H 'X-API-Key: your_api_key_here'
''')
async def get_friend_list_with_rank(session: SessionDep, user_id: str, api_key: str = Depends(verify_api_key)):
    try:
        UUID(user_id, version=4)
    except ValueError:
...
@router.get("/globalrank", response_model=list[UserStatisticLeaderboardEntry], dependencies=[Depends(verify_api_key)], summary="An secured API endpoint, get global rank leaderboard", description='''Retrieve the global rank leaderboard. Requires a valid API key and client ID..
To obtain an API key and client ID, use the `/users/apikey` endpoint. Include the following headers in your request:
- `X-API-Key`: Your API key
- `X-Client-ID`: Your client ID
Example:
...
curl -X 'GET' \n
  ...
  -H 'X-API-Key: your_api_key_here'
''')
async def get_global_rank(session: SessionDep, api_key: str = Depends(verify_api_key)):
    global_rank=userStatisticService.get_global_leaderboard(session=session, current_user=None)
    return global_rank


@router.get("/gamehistory/{user_id}", response_model=list[UserGameDetail], dependencies=[Depends(verify_api_key)], summary="An secured API endpoint, get specific user game history", description='''Retrieve the game history of a specific user. Requires a valid API key and client ID.
To obtain an API key and client ID, use the `/users/apikey` endpoint. Include the following headers in your request:
- `X-API-Key`: Your API key
- `X-Client-ID`: Your client ID
Example:
...
curl -X 'GET' \n
  ...
  -H 'X-API-Key: your_api_key_here'
''')
async def get_game_history(session: SessionDep, user_id: str, api_key: str = Depends(verify_api_key)):
    try:
        UUID(user_id, version=4)
    except ValueError:
        raise APIError(status_code=400, code=APIErrorCode.BAD_REQUEST, msg="Invalid user ID format. Must be a valid UUID.")
