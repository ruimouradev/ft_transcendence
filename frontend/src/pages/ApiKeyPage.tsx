...
export default function ApiKeyPage() {
  ...
  const loadApiKeyStatus = async () => {
    try {
      ...
      const response = await axios.get<ApiKeyStatus>("/api/v1/users/apikey");
  ...
  const generateApiKey = async () => {
    try {
       ...
       * IMPORTANT:
       *
       * The API key is stored only in React state.
       * It will disappear when the page is refreshed/unmounted.
       */
